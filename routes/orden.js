...
router.post("/", (req, res) => {
  ...
  if (authHeader) {
    ...
    jwt.verify(process.env.TOKEN_SECRET,token, token, function(err, decoded) {
      console.log(err,decoded);
    });
    jwt.verify(process.env.TOKEN_SECRET,token, (err, user) => {
      console.log(process.env.TOKEN_SECRET,token,user);
      if (err) {
